// CACTIgold (Travis Scott) Tracker - Link Extractor
// HOW TO RUN:
// 1. Go to script.google.com, New project, paste this file
// 2. Click Run -> extractLinks, approve permissions
// 3. Check Google Drive for "cactigold-links.csv"

var SHEET_ID = '1gJqbQrb3dIWF-PLMsKkNUrftpQb8zxsZFDAIpSvT5Fo';

var TABS = [
  { name: 'unreleased', gid: 1807066929 },
  { name: 'released',   gid: 1137563349 },
  { name: 'recent',     gid: 846204501 },
  { name: 'fakes',      gid: 1615755984 },
  { name: 'stems',      gid: 1984232945 },
  { name: 'tracklists', gid: 1956640286 }
];
...
function extractLinks() {
  ...
  for (var t = 0; t < TABS.length; t++) {
    ...
    for (var h = 0; h < headers.length; h++) {
      ...
      if (nameIdx < 0 && headers[h].indexOf('title') === 0) nameIdx = h;
    ...
    for (var row = 0; row < values.length; row++) {
      ...
      if (era.indexOf('\n') >= 0) continue;
      if (eraLower.indexOf('last updated') >= 0) continue;
      if (eraLower.indexOf('notice') >= 0) continue;
      if (eraLower.indexOf('hover over') >= 0) continue;
  ...
  var file = DriveApp.createFile('cactigold-links.csv', csvContent, MimeType.CSV);
