...
import { ArrowLeft, X, Check, ShieldCheck, Copy, KeyRound, Lock, Wrench, Clock } from 'lucide-react';
...
// ─── Maintenance Gate ───────────────────────────────────────────────────────
// yeditsgold is temporarily closed to the public. Visitors see a maintenance
// screen; entering the access password reveals the real page. The unlock is
// remembered in localStorage so authorized people don't have to re-enter it.
//
...
// secrets.
const YEDITS_GATE_PASSWORD = 'Sophiachiconruiz';
const YEDITS_GATE_STORAGE_KEY = 'yeditsgold_maintenance_unlocked';

function isYEditsUnlocked(): boolean {
  try { return localStorage.getItem(YEDITS_GATE_STORAGE_KEY) === '1'; } catch { return false; }
}
...
function MaintenanceGate({ onUnlock }: { onUnlock: () => void }) {
  ...
  const submit = () => {
    if (password === YEDITS_GATE_PASSWORD) {
      try { localStorage.setItem(YEDITS_GATE_STORAGE_KEY, '1'); } catch {}
      onUnlock();
    } else {
      setError(true);
    }
    ...
    <div style={{ minHeight: '100vh', background: 'radial-gradient(circle at 50% 0%, #1a1508 0%, #0a0a0a 55%)', color: '#fff', fontFamily: "'Inter', system-ui, sans-serif", WebkitFontSmoothing: 'antialiased', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, paddingTop: 'env(safe-area-inset-top)' }}>
      <div style={{ width: '100%', maxWidth: 460, textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: 30, fontWeight: 900, letterSpacing: '-0.02em', lineHeight: 1.05, marginBottom: 12 }}>
          yedits<span style={{ color: ACCENT }}>gold</span> is under maintenance
        </h1>
        <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.55)', lineHeight: 1.6, marginBottom: 28 }}>
          We're doing some work behind the scenes. yeditsgold will be back and
          open to the public soon — thanks for your patience.
        </p>
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: '20px 22px', marginBottom: 28, textAlign: 'left' }}>
          ...
          <div style={{ position: 'relative', height: 8, borderRadius: 999, background: 'rgba(255,255,255,0.08)', overflow: 'hidden', marginBottom: 12 }}>
            <div style={{ position: 'absolute', inset: 0, width: '45%', borderRadius: 999, background: `linear-gradient(90deg, ${ACCENT}55, ${ACCENT})`, animation: 'yeditsMaintPulse 1.8s ease-in-out infinite' }} />
          </div>
      ...
      <style>{`@keyframes yeditsMaintPulse { 0% { transform: translateX(-110%); } 100% { transform: translateX(260%); } }`}</style>
    </div>
  );
}
...
export function YEditsGoldPage() {
  const navigate = useNavigate();
  const [unlocked, setUnlocked] = useState(isYEditsUnlocked);
  ...
  if (!unlocked) {
    return <MaintenanceGate onUnlock={() => setUnlocked(true)} />;
  }
