...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    if (!context.env.YEDITS_BUCKET) return json({ error: 'Storage not configured' }, 500);
    ...
    // Move objects one at a time, streaming the body straight into the new key
    // so large albums don't buffer every track into memory at once.
    for (const oldKey of keysToMove) {
      ...
      if (!obj) continue;
      await context.env.YEDITS_BUCKET.put(newKey, obj.body, { httpMetadata: obj.httpMetadata });
